...
struct E2EConfigLiveReloadTests {
    ...
    // MARK: - Feature 8: Filesystem Config Hot-Reload

    @Test("F08: ConfigWatcher lifecycle startWatching and stopWatching")
    func test_f08_configWatcherLifecycle() {
        let watcher = ConfigWatcher(debounceInterval: 0.05)
        let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("watcher_test_\(UUID().uuidString).json")
        try? "{}".write(to: tempURL, atomically: true, encoding: .utf8)
        defer { try? FileManager.default.removeItem(at: tempURL) }

        #expect(watcher.isWatching == false)

        watcher.startWatching(path: tempURL.path) { _ in }
        #expect(watcher.isWatching == true)
        #expect(watcher.watchedPath == tempURL.path)
    ...
    @Test("F08: ConfigWatcher detects file modification and triggers callback")
    func test_f08_configWatcherFileModificationDetection() async throws {
        let watcher = ConfigWatcher(debounceInterval: 0.05)
        let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("watcher_mod_\(UUID().uuidString).json")
        try "{}".write(to: tempURL, atomically: true, encoding: .utf8)
        defer {
            watcher.stopWatching()
        ...
        let changeBox = ConfigBox()

        watcher.startWatching(path: tempURL.path) { config in
            changeBox.set(config)
        }

        // Update file content with custom window width
        let updatedJSON = """
        {
            "window": {
                "width": 950
        ...
        """
        try updatedJSON.write(to: tempURL, atomically: true, encoding: .utf8)
    ...
    @Test("F08: ConfigWatcher manual trigger invokes reload")
    func test_f08_configWatcherManualTrigger() async throws {
        let watcher = ConfigWatcher(debounceInterval: 0.05)
        let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("watcher_manual_\(UUID().uuidString).json")
        try "{}".write(to: tempURL, atomically: true, encoding: .utf8)
        defer {
            watcher.stopWatching()
            try? FileManager.default.removeItem(at: tempURL)
    ...
    func test_f08_configWatcherDebounceSuppression() async throws {
        let watcher = ConfigWatcher(debounceInterval: 0.15)
        ...
        // Perform 5 rapid synchronous writes in immediate succession without yielding
        for i in 1...5 {
            let json = """
            { "window": { "width": \(700 + i) } }
            """
