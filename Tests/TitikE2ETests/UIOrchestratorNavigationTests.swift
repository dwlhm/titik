...
@Suite("UIOrchestrator Navigation Tests", .serialized)
@MainActor
struct UIOrchestratorNavigationTests {
    ...
    func testSpaceDelimitedPluginActivationInUIOrchestrator() {
        ...
        // Note bang contract (ADR 5)
        PluginHost.shared.registerNativePlugin(
            NotesPlugin(context: PluginContext(pluginId: NotesPlugin.id)),
            manifest: notesPluginManifest
        )
        orchestrator.performSearch("!note")
        #expect(orchestrator.activePluginUI == nil)

        orchestrator.performSearch("!note ")
        #expect(orchestrator.activePluginUI != nil)
        #expect(orchestrator.activePluginUI?.pluginId == "titik.plugin.notes")
    }
    ...
    @Test("CalculatorPlugin clean identifier bang activation with space")
