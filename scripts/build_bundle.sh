...
APP_VERSION="${APP_VERSION:-$(git describe --tags --abbrev=0 2>/dev/null | tr -d 'v' || echo "0.3.1")}"
if [ "$APP_VERSION" = "0.3.0" ] || [ -z "$APP_VERSION" ]; then
    APP_VERSION="0.3.1"
fi
...
<dict>
    ...
    <string>${APP_VERSION:-0.3.1}</string>
    <key>CFBundleVersion</key>
    <string>${APP_BUILD_NUMBER:-1}</string>
